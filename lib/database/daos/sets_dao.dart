...
class SetsDao extends DatabaseAccessor<AppDatabase> with _$SetsDaoMixin {
  ...
  Future<List<PersonalRecordEntry>> getAllPersonalRecords() async {
    final results = await customSelect(
      ...
      'MAX(ws.weight_kg * (1.0 + CAST(ws.reps AS REAL) / 30.0)) AS max_1rm '
      'FROM workout_sets ws '
      'JOIN exercises e ON ws.exercise_id = e.id '
      'WHERE ws.reps > 0 AND ws.weight_kg > 0 '
  ...
  /// Personal record (max weight) for an exercise across all sessions
